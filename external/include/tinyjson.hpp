...
namespace tiny {
    ...
    template <> inline void Value::Set(std::string v)
    {
        std::ostringstream oss;
        if (nokey_) {
            oss << "\"" << v << "\"";
    ...
    template <> inline void Value::Set(const char* v) { Set(std::string(v)); }
     ...
     *
     */
    template <typename T> class ValueArray : public T {
    public:
        ValueArray() {}
        ...
        int Count() { return static_cast<int>(vo_.size()); }
    ...
    inline bool ParseJson::ParseArray(std::string json, std::vector<std::string>& vo)
    {
        ...
        if (!tokens.empty()) { vo.push_back(tokens); }
        return true;
    }
    ...
    inline bool ParseJson::ParseObj(std::string json)
    {
        ...
        if (keyval_.size() == 0) { keyval_.push_back(json); }
        return true;
    }
    ...
    public:
        ...
        template <typename R> R Get(std::string key, R defVal)
        {
        ...
        template <typename R> R Get(std::string key) { return Get(key, R()); }

        template <typename R> R Get() { return Value(KeyVal_[0]).GetAs<R>(); }
    ...
    template <> inline xarray TinyJson::Get(std::string key)
    {
