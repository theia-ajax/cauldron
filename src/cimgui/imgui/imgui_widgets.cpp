    ...
    float power)
{
    ...
    if (power != 1.0f) {
        IM_ASSERT(
            power == 1.0f
            && "Call function with ImGuiSliderFlags_Logarithmic flags instead of using the old "
...
#define STB_TEXTEDIT_MOVEWORDRIGHT STB_TEXTEDIT_MOVEWORDRIGHT_IMPL
        ...
        int new_text_len)
    {
        ...
        if (pos != text_len)
            memmove(
                text + pos + new_text_len, text + pos, (size_t)(text_len - pos) * sizeof(ImWchar));
    ...
    void* callback_user_data)
{
    ...
    IM_ASSERT(
        !((flags & ImGuiInputTextFlags_CallbackHistory)
          && (flags & ImGuiInputTextFlags_Multiline))); // Can't use both together (they both use
                                                        // up/down keys)
    IM_ASSERT(
        !((flags & ImGuiInputTextFlags_CallbackCompletion)
          && (flags & ImGuiInputTextFlags_AllowTabInput))); // Can't use both together (they both
                                                            // use tab key)
    ...
    const bool init_make_active =
        (focus_requested || user_clicked || user_scroll_termsh || user_nav_input_start);
...
bool ImGui::CollapsingHeader(const char* label, bool* p_open, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_open != NULL) {
        ...
        if (CloseButton(close_button_id, ImVec2(button_x, button_y))) *p_open = false;
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
                                     ...
                                     // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        {
            want_open = true;
        } else if (g.NavId == id && g.NavMoveRequest && g.NavMoveDir == ImGuiDir_Down) // Nav-Down
                                                                                       // to open
        {
            want_open = true;
            NavMoveRequestCancel();
